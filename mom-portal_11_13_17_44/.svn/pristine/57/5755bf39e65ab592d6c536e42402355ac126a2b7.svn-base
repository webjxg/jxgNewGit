            ...
            require(['jqGrid_my'], function (jqGridAll) {
                if (statuName == 'GCWLHS') {
                    ...
                    for (var i = 0; i < result.length; i++) {
                        ...
                        var isrowNumbers={rownumbers:true};
                        var ishide = {hiddengrid: true};//是否折叠
                        var lastsel;
                        var gridEdit = jqGridAll.jG_editRowFn('#' + tableId, lastsel,true);
                        //默认第一个不折叠
                        if (i == 0) {
                            $('#' + tableId).jqGrid($.extend(configData, gridConfig, gridEdit, gridsum,isrowNumbers));
                        } else {
                            $('#' + tableId).jqGrid($.extend(configData, gridConfig, gridEdit, gridsum, ishide,isrowNumbers));
                        }
                    ...
                    //默认第一个补折叠
                    $('#' + statuName).jqGrid($.extend(gridEdit, configData, gridConfig,isrowNumbers));
