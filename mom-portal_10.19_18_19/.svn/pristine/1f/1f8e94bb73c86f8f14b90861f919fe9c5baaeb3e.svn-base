        ...
        listInit: function(){
            ...
            require(['easyui_my'],function(easyuiObj){
                function btncilck(){
                    ...
                    $('#delete-btn').click(function(){
                        ...
                        if(str.length>0){
                            Mom.layConfirm('要删除该菜单及所有子菜单项吗？',function(layerIdx, layero){
                                Api.ajaxForm(Api.admin + '/api/sys/SysMenu/ajaxDel', {ids:str.substr(1)}, function(result){
                    ...
                    $('.btn-change').unbind().click(function () {
                        var id = $(this).attr('id');
                        Bus.openEditDialog('修改菜单','systemSettings/menuInner.html?id='+id,'800px','456px', saveCallback);
                    });
                    //删除
                    $('.btn-delete').unbind().click(function () {
                        var id = $(this).attr('id');
                        Bus.deleteItem('要删除该菜单及所有子菜单项吗', Api.admin + '/api/sys/SysMenu/ajaxDel',id,deleteItemCallBack);
                    });
                    // 添加下级菜单
                ...
                //删除数据
                function deleteItemCallBack(result, layerIndex,id){
                    if(result.success == true){
                        easyuiObj.tg_removeTreegridNode('#tt',id);
                    }else{
                        Mom.layAlert(result.message);
                ...
                function pageLoad(node) {
                    Api.ajaxJson(Api.admin+"/api/sys/SysMenu/getTreeGraidJson",{},function(result){
                        if(result.success){
                            ...
                            btncilck();
                            renderIChecks($('.datagrid-header-check').children('input'),$('.datagrid-cell-check').children('input'));//
                        }else{
            ...
            window.operFmt = function(value, row){
                return "<a class='btn btn-check' id='"+ row.id+"'><i class='fa fa-search-plus'></i>查看</a>" +
                    " <a class='btn btn-change' id='"+ row.id+"'><i class='fa icon-change'></i>修改</a>" +
                    " <a class='btn btn-delete' id='"+ row.id+"'><i class='fa fa-trash'></i> 删除</a>"  +
