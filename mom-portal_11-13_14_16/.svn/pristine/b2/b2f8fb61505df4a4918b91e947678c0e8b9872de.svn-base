    ...
    var jqGridAll = {
       ...
       jqGridTable: function(tableId,settings){
        ...
        jqGridTableLevel: function(tableId,settings1,settings2,config,subtable,callback){
            ...
            var setting = jqGridAll.setting(settings1);
            ...
            function bindSubGrid(subgrid_id, collectLineId) {
                ...
                $("#" + subgrid_id).html(
                    "<div class='subTableBox'><table id='"+subgrid_table_id+"' class='scroll'></table><div id='"+subgrid_pager_id+"' class='scroll'></div></div>");
                var dataJsons = {
                    id : collectLineId
                };
                ...
                if(!config.contentType){
                    Api.ajaxJson(url,dataJsons,function(result){
                        var setting = jqGridAll.setting(settings2);
                        setting.data = result.rows;
                        /*if(settings2){
                        ...
                        if(callback){
                            callback(subtable,dataJsons,result);
                            /*↑↑↑↑↑↑↑贾旭光加了一个传回的参数 当时请求时的参数再返回去 可以拿到相应表的参数*/
                        }

                    });
                }else{
                    Api.ajaxForm(url,dataJsons,function(result){
                        var setting = jqGridAll.setting(settings2);
                        setting.data = result.rows;
                        /*if(settings2){
                            setting = $.extend(true, {},setting, settings2);
