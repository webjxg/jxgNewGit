...
require(['/js/zlib/app.js'],function (app) {
    var PageModule = {
         ...
         createMaterial:function(data,mainItem){
            ...
            if(len>0){
                $.each(data,function(i,o){
                    htmls += "<li  data-mtrlId='"+o.mtrlId+"' data-enable='"+o.mtrl.enable
                        +"'>"+o.mtrl.mtrlName+"</li>";
                });
                $("."+mainItem).append(htmls);
                $("."+mainItem+" li").click(function () {
                    $(this).addClass("active").siblings("li").removeClass("active");
                });
            }else{
                $("."+mainItem).append(PageModule.nodataHtml);
            }
        },
        //创建罐列表
         createList:function (data,item) {
            ...
            if(len>0){
                var htmls = '';
                $.each(data,function(i,o){
                   htmls += "<li data-id='"+o.id+"'>"+o.nodename+"</li>" ;
                });
                $("."+item).html(htmls);
            }else{
            ...
            $("#btn-left").unbind("click").on("click",function () {
                ...
                if(len<=0 && dataList == undefined){
                    ...
                    if($(".advocate .active").length<=0){
                        ...
                        var exist = PageModule.criteria(dataList,mtrlId);
                        var name = $(".advocate .active").text();
                        if(exist){
                            var obj = {
                                nodeId:nodeId,
        ...
        createAllocated:function (data) {
            ...
            if(len>0){
                $.each(data,function(i,o){
                   htmls +=  "<li data-id='"+o.id+"' data-nodeId='"+o.nodeId+"' data-mtrlId='"+o.mtrlId+"'>" +o.mtrl.mtrlName;
                    if(o.mtrl.enable == "1"){
                ...
                $("#allocated").html(htmls);
                $(".allocated li").click(function () {
                    $(this).addClass("active").siblings("li").removeClass("active");
                });
                PageModule.clickHandler(data);
            }else{
                $("#allocated").html(PageModule.nodataHtml);
            }
        },
