...
require(['/js/zlib/app.js'], function(App) {
    require(['jqGrid_my'], function (jqGridAll) {
        ...
        tabBtnclick:function (temPlate) {
            ...
            $(".tab-nav li").each(function (index,item) {
                $(item).unbind("click").on("click",function () {
                    ...
                    $(".jqGridTable-item").eq(index).addClass("active").siblings(".jqGridTable-item").removeClass("active");
        ...
        * */
        createPrimary:function (index,dataTable,temPlate){
           ...
           var config = {
               ...
               contentType:"Form"
                ...
                jqGridAll.jG_loadTable("#TankChk",dataTable);
                $("#measure .jqGridTable-info").removeClass("hide");
                $('.TankSeal-num').text(dataTable.length);
                function clickBtn() {
                    $(".delete-btn").unbind("click").on("click",function (item) {
        ...
        createBatch:function (tableArr,template,status) {
            ...
            jqGridAll.jG_loadTable("#TankChk",tableArr)
        },
            //判断选择时间是再班次时间内
            /*metaType:检尺类型
            *initTime：列表的时间
            ...
            renderPotTable: function (potArr, moveArr, result) {
                require(['jqGrid_my'], function (jqGridAll) {
                    ...
                    var optionsMMove = {
                        ...
                        //     $(".ui-jqgrid-htable").jqGrid('resetSelection');
                        //     return(true);
                        // }
                    };
                    ...
                    $('.TankSeal-num').text(result.rows.length);
                    //添加按钮
                    $('#btn-add').unbind('click').on('click', function () {
              ...
              function load(){
                  ...
                  Api.ajaxForm(Api.mtrl +"/api/mv/TankSeal/page",dataList,function (result) {
                      ...
                      if(result.success){
                          ...
                          $('.jqGridTable-num').text(result.rows.length);
                          //提交按钮
    ...
    $(function () {
        if($("#tankManage").length>0){   //初始化列表加载
            PageModule.init();
        }else if($("#initializeFrom").length>0){       //检尺初始化
