   ...
   var PageModule = {
       ...
       loadData:function (url,data,itemArr) {
           Api.ajaxForm(url,data,function (result) {
               Bus.appendOptionsValue('#fctrName',result.fctrList,'id','fctrName');  //加载仓库中工厂下拉
               Bus.appendOptionsValue('#fctrName2',result.fctrList,'id','fctrName');  //加载物料中工厂下拉
       ...
       createInitList:function (item,rows) {
           $("#"+item).empty();
           var len = rows.length,htmls = "";
           if(len>0){
               if(item == "list1"){
                   $.each(rows,function(i,o){
                       htmls += "<li data-id='"+o.id+"'>"+o.areaName+"</li>";
                   });
                   $("#"+item).append(htmls);
               }else{
                   $.each(rows,function(i,o){
                      htmls += "<li data-id='"+o.id+"' data-enable='"+o.enable+"'>"+o.mtrlName+"</li>";
                   });
                   $("#"+item).append(htmls);
               }
           }else{
               $("#"+item).html(PageModule.nodataHtml);
           }
       ...
       createAllocated:function (data,item) {
           ...
           if(len>0){
               for (var i = 0; i < data.length; i++) {
                   htmls += "<li data-id='" + data[i].id + "' data-nodeId='" + data[i].nodeAreaId + "' data-mtrlId='" + data[i].mtrlId + "'>" + data[i].mtrl.mtrlName;
       ...
       clickHandler:function (dataList) {
           ...
           //上移按钮
            ...
            $("#btn-save").unbind("click").on("click",function () {
                   if($("#list1 .active").length<=0 || dataList == undefined){
                       Mom.layMsg("请选择仓库");
                   }else{
                       var nodeAreaId = $("#list1 .active").attr("data-id");    //仓库ID
                       for(var i=0;i<dataList.length;i++){
                           dataList[i].nodeAreaId = nodeAreaId;
                       }
                       if(dataList.length<=0){                                 //如果为空的话
                           var msg = {};
                           msg.nodeAreaId = $("#list1 .active").attr("data-id");
                           dataList.push(msg);
                       }
                        for(var j=0;j<dataList.length;j++){                  //物料类型
           ...
           //左移按钮
            $("#btn-left").unbind("click").on("click",function () {
                var len = $("#list2 .active").length;  //物料是否选中
