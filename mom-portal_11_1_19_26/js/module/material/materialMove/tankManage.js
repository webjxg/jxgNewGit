                        ...
                        $(subtr).each(function (i, o) {
                            $(this).on('dblclick', function () {
                                ...
                                if (moveType != '罐付罐') {
                                    ...
                                    Bus.openEditDialog('新建物料移动信息', 'material/materialMove/addMove.html?potDate=' + potDate + '&classes=' + escape(classes) + '&tankId=' + tankId, '681px', '495px', function () {
                ...
                require(['datetimepicker', 'Page'], function () {
                    ...
                    if (moveId == null && potId != null) {
                        ...
                        $("#startTime").val("").datetimepicker({
                            ...
                            autoclose: true,  //选择时间后自动隐藏
                            clearBtn: true,
                            startDate: moveDate + ' ' + startTime,
                            endDate: moveDate + ' ' + endTime
                        ...
                        $('#operationType').on('change', function () {
                            ...
                            if (moveTypeS == 'TANK_MOD' || moveTypeS == 'TANK_RETO_STOREHOUSE') {
                                ...
                                attrInit('table tr td:nth-of-type(2)', '#operationType,#ownMaterials,#ownMaterial,#startTime', '#startTime');
                                nodechange(true);
                                openVal('.dlvVal', 'add', '.rcvVal', potId);
                            }
                            //交库||复尺
                            if (moveTypeS == 'TANK_TO_STOREHOUSE' || moveTypeS == 'TANK_RE_CHK') {
                                ...
                                if (moveTypeS == 'TANK_RE_CHK') {
                                    ...
                                    $('#ownMaterial').remove();
                                    attrInit('table tr td:nth-of-type(2)', '#operationType,#ownMaterial,#ownMaterials,#startTime', false);
                                    openVal('.dlvVal', 'add', '.rcvVal', potId);
                                }
                            }
                        });
                    ...
                    function openVal(selector, typeVal, closeSel, tankId) {
                        ...
                        $(closeSel).unbind('click', function () {
                            $(this).find('input:text').css('cursor', 'auto');
                        })
                    }
                                ...
                                Api.ajaxJson(Api.mtrl + '/api/mv/TankMtrlMove/getListByType', JSON.stringify(data), function (result) {
                                    if (result.success) {
                                        if (result.NodeList[0] != undefined) {
                                            ...
                                            rcvMtrl('#oppositeNode', result.NodeList, '#oppositeMaterial', '#oppositealias', true);
                                        } else {
                                            $('#oppositeNode,#oppositeMaterial,#oppositealias').val('');
                                            Mom.layMsg('对方物料无信息')
                                        }
                            ...
                            if (result.success) {
                                ...
                                if (isDlv == true) {
                                    ...
                                    $(result.mtrlList).each(function (i, o) {
                                        ...
                                        $(result.mtrlList).each(function (i, o) {
                                            var idchange = $('#ownMaterials').val();
                                            if (idchange == o.id) {
                                                $('#ownalias').val(result.mtrlList[i].mtrlAlias);
                                            }
                                            $('#ownMaterials').on('change',function () {
                                                var change = $('#ownMaterials').val();
                                                if (change == o.id) {
                                                    $('#ownalias').val(result.mtrlList[i].mtrlAlias);
                                                }
                                    ...
                                    if (result.NodeList == undefined && result.mtrlList == undefined) {
                                        if ($('select#ownMaterials').length > 0) {
                                            ...
                                            $('#ownalias').val(result.nodeMtrlMove.dlvMtrl.mtrlAlias)
                                        }
                                        //正常操作
                                        ...
                                        $('#oppositeNode,#oppositeMaterial,#oppositealias').val('').empty();
                                    }
                                }
                    ...
                    function rcvMtrl(parSel, parRows, midSel, selector, ischange) {
                        //默认进来的时候
                        var idsub;
