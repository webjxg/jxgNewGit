require(['/js/zlib/app.js'], function (App) {
    var PageModel = {
        ...
        createTable:function (tableArr,tempArr,data,index) {
                ...
                if(data.type == "Mineral"){     //创建矿石石灰表
                    ...
                    for(var i=0;i<tableArr.length;i++){
                        ...
                        //找到与tableArr中对应code的模板信息
                                colNames = tempArr[i].colNames;   //表名
                                colModel = tempArr[i].colModel;   //对应字段
                                dataTable = tableArr[i].nodes;    //渲染数据
                            //设置排序
                            var dataArr = PageModel.sortFn(dataTable);
                        if(i==0){   //如果是第一个table，就配置编辑时不可以输入汉字
                            ...
                            var editRowFn = jqGridAll.jG_editRowFn("#dataTable0",'');//设置可编辑，但是不可以输入汉字
                            ...
                            jqGridAll.jG_Resize("#dataTable"+i,"#tableArr"); //根据屏幕大小改变表格
                        }
                        if(i==1){
                            var editRowFn = jqGridAll.jG_editRowFn("#dataTable1",'');//设置可编辑，但是不可以输入汉字
                            var configData  = jqGridAll.jG_configData(dataArr);  //创建table的数据
                        ...
                        if(i==0){
                            $(".tabBtn li").eq(i).addClass("active");
                    ...
                    for (var i = 0; i < tableArr.length; i++) {
                        ...
                        for(var k=0;k<tempArr.length;k++){
                            // if (tableArr[i].code == tempArr[k].itemCode) {
                                ...
                                dataTable = tableArr[k].nodes;
                            // };
                            //设置排序
                            var dataArr = PageModel.sortFn(dataTable)
                        }
                        var editRowFn = jqGridAll.jG_editRowFn("#dataTable"+i,'');//设置可编辑，但是不可以输入汉字
                        var configData = jqGridAll.jG_configData(dataArr);  //创建table的数据
                        var len = dataArr.length;
                    ...
                    for (var i = 0; i < tableArr.length; i++) {
                        ...
                        //设置排序
                        var dataArr = PageModel.sortFn(dataTable);
                        var editRowFn = jqGridAll.jG_editRowFn("#dataTable",'');//设置可编辑，但是不可以输入汉字
                        var len = dataArr.length;
                        var configData  = jqGridAll.jG_configData(dataArr);  //创建table的数据
                        var gridConfig = jqGridAll.jG_config('',colNames,colModel,len);
                        $("#dataTable").jqGrid($.extend(configData,gridConfig,editRowFn));
                        jqGridAll.jG_Resize("#dataTable","#tableArr"); //根据屏幕大小改变表格
                    }
                    ...
                    for (var i = 0; i < tableArr.length; i++) {
                        ...
                        var dataArr = PageModel.sortFn(dataTable);
                        var len =dataArr.length;
                        var editRowFn = jqGridAll.jG_editRowFn(itemHtml,'');//设置可编辑，但是不可以输入汉字
                        var configData  = jqGridAll.jG_configData(dataArr);  //创建table的数据
                        var gridConfig = jqGridAll.jG_config('',colNames,colModel,len);
                        $(itemHtml).jqGrid($.extend(configData,gridConfig,editRowFn));
                        jqGridAll.jG_Resize(itemHtml,"#tableArr"); //根据屏幕大小改变表格
                    ...
                    for (var i = 0; i < tableArr.length; i++) {
                        ...
                        var editRowFn = jqGridAll.jG_editRowFn(tableItem,'');//设置可编辑，但是不可以输入汉字
                        var configData  = jqGridAll.jG_configData(dataArr);  //创建table的数据
                        var gridConfig = jqGridAll.jG_config('',colNames,colModel,len);
                        $(tableItem).jqGrid($.extend(configData,gridConfig,editRowFn));
                        jqGridAll.jG_Resize(tableItem,"#tableArr"); //根据屏幕大小改变表格
            ...
            $(".btn-save").unbind("click").on("click",function () {
                ...
                if($(".supervisionplate").val() == ""){
                    ...
                    Mom.layAlert("请填写参与监盘监盘人员")
                }
                if(data.type == "Process"){
                    if($("#classes option:selected").val() == ""){
                        Mom.layAlert("请选择班次")
                    }else if($("#teamGroup option:selected").val() == ""){
                        Mom.layAlert("请选择班组")
                    }else{
                        var ids=[];
                        ids.push("dataTable");
        ...
        createTabClick:function (tableArr) {
            //先创建tab切换按钮
            for(var i=0;i<tableArr.length;i++){
                var li = $("<li type='"+tableArr[i].code+"'>"+tableArr[i].name+"</li>");
                $(".tabBtn").append(li);
            };
        ...
        eachTable:function (item,items,changeTitle) {
            ...
            $("#"+item +" tr").each(function (index,items) {
                if(changeTitle == $(items).find("td").eq(3).text()){    //如果说选中编辑的td==所有tr下的第三个td
                    if($(items).find("td:last").find("input").val() == undefined){  //如果最后一个td的input不存在
                        num+=parseInt($(items).find("td:last").text());
                    }else{
                        $(items).find("td:last").find("input").val();      //如果input存在
                        num1 = parseInt($(items).find("td:last").find("input").val());
                    }
                    sum=num+num1;
                };
            });
        ...
        dataCollection:function (ids,obj,dataArr,tempArr) {
            ...
            require(["jqGrid_my"],function (jqGridAll) {
                ...
                Api.ajaxForm(Api.pi+"/api/PiApi/tagNearLocal",data,function (result) {
                    ...
                    PageModel.createTable(dataArr,tempArr,obj,1)
                });
            })
        },
        //计算
