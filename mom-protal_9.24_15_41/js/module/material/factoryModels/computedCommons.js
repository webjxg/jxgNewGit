define([''], function () {
    Mom.include('computed_css', '/js/module/material/factoryModels/', [
        'computedCommons.css'
    ]);
    var computedFn = {
        /**切换tab
        ...
        renderChange:function (innerbox,contentbox,liList,callback) {
            $(innerbox).find(liList).each(function (e) {
                ...
                $(this).unbind('click').on('click', function () {
                    computedFn.ajaxjson('../../../../json/factoryModels/material.json', renderMaterials, 0, e)
                })
            });
            ...
            $('.tabChange').find('li').each(function (i) {
                $(this).unbind('click').on('click', function () {
                    ...
                    $(contentbox).eq(i).find(liList).each(function (e) {
                        //到时候要传参数
                        $(this).unbind('click').on('click', function () {
                            computedFn.ajaxjson('../../../../json/factoryModels/material.json', renderMaterials, i, e);
                        });
                    });
            ...
            function renderMaterials(result, i, e) {
