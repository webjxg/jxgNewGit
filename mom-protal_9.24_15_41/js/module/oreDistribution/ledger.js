require(['/js/zlib/app.js'], function (App) {
    var PageModule = {
        ...
        init: function () {
            ...
            $('#caseId').val(caseName);
            PageModule.renderSelect(Api.aps + '/api/ob/LedgerAccount/getOreType', caseId, "#oreType", 'oreType', 'oreTypeName');
            ...
            require(['Page'], function () {
                window.pageLoad = function () {
                    ...
                    //渲染表格以及渲染分页
                    Page.init(Api.aps + "/api/ob/LedgerAccount/page", data, true, function (rows) {
        ...
        //渲染下拉矿石种类
        renderSelect: function (url, caseId, appendEl, valueField, labelFile, callback) {
                    ...
                    $('#dataTable').dataTable({
                        ...
                        "aoColumns": [
                            ...
                            {"data": "oreDate", 'sClass': "alignCenter old ", 'sWidth': '12%'},
                            {
                                ...
                                "render": function (data, type, row, meta) {
                                    var data = "<select type='text' name='oreType' class='oreType form-control editor editText select2 '></select>";
                                    return data;
                            ...
                            },
                            {
                                ...
                                "render": function (data, type, row, meta) {
                                    return data = "<input type='text' name='beltWeight' class='beltWeight form-control editText editor  '>"
                                }
                            },
                            {
                                "data": "avgNum", 'sClass': "alignCenter old", 'sWidth': '8%',
                                "render": function (data, type, row, meta) {
                                    return data = "<input type='text' name='avgNum' class='avgNum form-control editor editText'>";
                                }
                            },
        ...
        dataColinit: function () {
            ...
            require(['Page'], function () {
                window.pageLoad = function () {
                    ...
                    //渲染表格以及渲染分页
                    Page.init(Api.aps + "/api/ob/LedgerAccount/collectionData", data, true, function (rows) {
                        PageModule.renderTableSon(rows);
                        PageModule.getParVal(rows);
        ...
        /*遍历子页面的ichecks*/
        getParVal: function (rows) {
            for (var i = 0; i < rows.length; i++) {
                var ichkpar = $(top.window['iframe42'].document).find('#datainner tr').find('input:checkbox');
                for (var j = 0; j < ichkpar.length; j++) {
                    if ($(ichkpar).eq(j).attr('laboratoryid') != '') {
                        if (rows[i].id == $(ichkpar).eq(j).attr('laboratoryid')) {
                            rows.splice(i, 1)
                        }
                    }
