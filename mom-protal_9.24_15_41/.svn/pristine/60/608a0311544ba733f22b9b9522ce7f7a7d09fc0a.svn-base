    ...
    var PageModule = {
        ...
        KeyWorkPVInit:function(){
            require(['easyui_my'], function (easyui) {
                var year = Mom.getUrlParam('year') || '某',
                    month = Mom.getUrlParam('month') || '某';
                    ...
                    if (result.success) {
                        ...
                        if (rowsLen > 0) {
                            $.each(result.rows, function (index, item) {
                                addGridDiv(index, item.key)
                                dataGrid('td' + index);
                                hideDatagrid('td' + index);
                                $('#td' + index).datagrid('loadData', item.value);//将数据渲染到Grid中
                                // tableArr.push(index);
                            });
                            Mom.setFrameHeight();
                            $('.noData').hide();
                ...
                $(window).resize(function () {
                    $(".tableItem").each(function (i) {
                        $('#td' + i).datagrid('resize');
                    });
                });

                //动态添加Grid
                function addGridDiv(index, title) {
                    var htmlStr = "<div class=\"tableItemBox\"><table id=\"td" +
                        index +
                ...
                function dataGrid(tableID) {
                    ...
                    $(tableID).datagrid({
                        ...
                        onClickRow: function (index) {
                            daleteGridId = tableID;
                            deleteColId = index;//记录当前选中的行
                            easyui.dg_startEditing(index, tableID);
                            clearOtherGridSelections(tableID);//清空其他Grid的选中和编辑
                        },
                        //创建table
                        columns: [[
                            {field: 'id', title: 'id', hideColumn: 'id', class: "iCheck-helper"},
                        ...
                        onAfterEdit: function (rowIndex, rowData, changes) {
                            ...
                            easyui.dg_mergeCells(loadedData, tableID, "typeWork");
                        },
                        //合并单元格
                        onLoadSuccess: function (data) {
                            loadedData = data;
                            easyui.dg_mergeCells(data, tableID, "typeWork");
                        },
                        toolbar: [{
                ...
                //隐藏指定Grid的['id','mainId']列
                function hideDatagrid(tableID) {
                    var tableID = "#" + tableID;
                    var hideColumnEle = ['id', 'mainId'];
                    $(hideColumnEle).each(function (index, item) {
                        $(tableID).datagrid('hideColumn', item);
                    });
                }
        ...
        KeyWorkPVEInit:function () {
            require(['easyui_my'], function (easyui) {
                require(['/js/dataGridCustom.js'], function () {
                    ...
                    $.ajax({
                        url: "../../json/user/monthvalue.json",//json文件位置
                        type: "GET",//请求方式为get
                        dataType: "json",
                    ...
                    $(".btn-delete").click(function () {
                        deleteDateRow(daleteGridId, deleteColId);
                    });
                    ...
                    function dataGrid(tableID) {
                        ...
                        $(tableID).datagrid({
                            ...
                            columns: [[
                                ...
                                {field: 'thoseResponsible', title: '责任人', editor: 'text', width: 50, align: 'center'},
                                {field: 'checkPeople', title: '检查人', editor: 'text', width: 50, align: 'center'},
                    ...
                    //删除指定一行数据
                    function deleteDateRow(tableID, index) {
                        var ids = '';
                        $.each(tableArr, function (i, item) {
                            var _tableId = '#td' + item;
                    ...
                    /*** 点击保存时调用方法*/
                    function postData() {
                        var editItems = easyui.dg_getAllData(tableIdArr);
                        var data = {
