var url, data, defaultVals;
window.setConfig=function(url_, data_, defaultVals_) {
    url=url_, data=data_, defaultVals=defaultVals_;
}
require(['/js/zlib/app.js'], function(App) {
    ...
    //加载ztree插件
    require(['ztree_all'], function(ZTree) {
        var ztree = new ZTree();
        window.loadDataFn = function (url, data, defaultVals) {
            Api.ajaxJson(url, data, function (result) {
                if (result.success) {
                    var treeObj = ztree.loadData($('#tree'), result.rows, multiple);
        ...
        //立即调用
        loadDataFn(url, data, defaultVals);
    });
