require(['/js/zlib/app.js'], function(App) {
    ...
    var PageModule = {
        ...
        //指令操作列表
        orders: function(){
            window.updatasons = function (node,level){
                var data={
                    "id":node
                };
            ...
            function clickButton(level) {
                $('.btn-addson').click(function(){
                    ...
                    Bus.openEditDialog('添加下级指令','./proDis/createInsCV.html?pid='+id+'&proc='+proc,'600px', '450px')
                });
                $('.btn-add').click(function(){
                    var id = $(this).parents("tr").find('.i-Checks').attr('id');
                    Bus.openEditDialog('添加指令','./proDis/createInsCV.html?&id='+id,'600px', '450px')
                });
                $('.btn-change').click(function () {
                    var id = $(this).parents("tr").find('.i-Checks').attr('id');
                    Bus.openEditDialog('修改指令', './proDis/createInsCV.html?level='+level+'&id=' + id, '600px', '450px')
                });
        ...
        check: function(){
            ...
            window.doSubmit = function(iframeWin, iframeBody, layerIdx){
                ...
                $("#proc").attr("disabled",false);//提交前移除disabled属性
