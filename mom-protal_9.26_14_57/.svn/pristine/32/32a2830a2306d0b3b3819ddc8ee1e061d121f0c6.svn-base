            ...
            require(['Page'], function () {
                ...
                window.pageLoad = function () {
                    var data = {
                        ...
                        heap: $("#heap option:selected").text() == "-- 全部 --" ? "" : $("#heap option:selected").text(),
                        status: $("#status option:selected").val()
                    };
                    //修改默认每页显示条数
                    Page.init(Api.aps + "/api/ob/OreBlendingCase/page", data, true, function (result) {
        ...
        createTable: function (tableDate) {
            $("#treeTable").dataTable({
                ...
                "aoColumns": [
                    ...
                    {"data": "oreName", 'sClass': "alignCenter "},
                    {"data": "heapNum", 'sClass': "alignCenter "},
                    {"data": "heap", 'sClass': "alignCenter"},
                    {
                        "data": "null", 'sClass': "alignCenter status",
                        "render": function (data, type, row, meta) {
                            var html = "";
                            var classSet = "";
                            if (row.status == "1") {
                    ...
                    {"data": "createUser", 'sClass': "alignCenter"},
                    {
                        ...
                        "render": function (data, type, row, meta) {
                            var html = "";
                            if (row.status == "0") {
                                html = "<a class='btn  btn-info btn-check' ><i class='fa fa-search-plus'></i>查看</a >" +
                                    "<a class='btn btn-Ledger btn-allotApp'><i class='fa fa fa-newspaper-o'></i>台账</a >";
                            } else {
                                html = "<a class='btn btn-check' ><i class='fa fa-search-plus'></i>查看</a >" +
                                    "<a class='btn btn-Ledger btn-allotApp'><i class='fa fa-newspaper-o'></i>台账</a >";
                            }
                            return html;
                        }
                    }
